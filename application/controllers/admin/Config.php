...
class Config extends CI_Controller {
	...
	public function change_password()
	{
		...
		if ( $this->form_validation->run() == TRUE ) {
			...
			$this->session->set_flashdata( 'msg', 'Contraseña editada correctamente' );
	...
	public function _password_verify( $old_password )
	{
		...
		if ( $user->password != $this->config_m->hash( $old_password ) ) {
			...
			$this->form_validation->set_message( '_password_verify', 'La contraseña anterior no coincide con la contraseña actual' );
